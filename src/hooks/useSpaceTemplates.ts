...
import { SpaceTemplate, LayoutNode, Mode } from "@/lib";
...
export function useSpaceTemplates() {
  ...
  const captureCurrent = useCallback(
    (
      ...
      const isDuplicate = currentTemplates.some(
        (t) =>
  ...
  const archiveTemplates = useCallback((ids: string[]) => {
    ...
    setTemplates((prev) =>
      prev.map((t) => (ids.includes(t.id) ? { ...t, isArchived: true } : t)),
    );
  ...
  const unarchiveTemplates = useCallback((ids: string[]) => {
    const count = ids.length;
    setTemplates((prev) =>
      prev.map((t) => (ids.includes(t.id) ? { ...t, isArchived: false } : t)),
    );
