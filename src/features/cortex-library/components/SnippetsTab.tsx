  ...
  const renderActiveContent = () => {
    if (filtered.length === 0) {
      return (
        <EmptyState
          icon={Terminal}
          title={searchQuery ? "No Active Snippets Found" : "No Active Snippets"}
            ...
            <TableRow key={snippet.id}>
              ...
              <TableCell>
                <div className="flex items-center gap-1.5">
                  <ChevronRightSquare size={10} className="text-[var(--text-secondary)]/50 shrink-0" />
                  ...
                  {onArchive && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon-xs" className="text-[var(--text-secondary)]/50 hover:text-[var(--accent-primary)] hover:bg-[var(--accent-primary)]/5" onClick={() => onArchive(snippet.id)}>
                          <Archive size={12} />
                        </Button>
                      </TooltipTrigger>
      ...
      <>
        {viewMode === 'card' ? (
          ...
          <Table>
            ...
            <TableBody>
              {archivedFiltered.map(snippet => (
                <TableRow key={snippet.id} className={cn(
                  "transition-all",
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-6 rounded bg-[var(--text-secondary)]/5 border border-[var(--border-color)]/50 flex items-center justify-center shrink-0">
                        <Terminal size={12} className="text-[var(--text-secondary)]/50" />
                      </div>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" size="icon-xs" className="text-[var(--text-secondary)]/50 hover:text-[var(--accent-primary)] hover:bg-[var(--accent-primary)]/5" onClick={() => onUnarchive && onUnarchive(snippet.id)}>
                            <RotateCcw size={12} />
                          </Button>
                        </TooltipTrigger>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" size="icon-xs" className="text-[var(--text-secondary)]/50 hover:text-red-400 hover:bg-red-500/10" onClick={() => onDelete(snippet.id)}>
                            <Trash2 size={12} />
                          </Button>
                        </TooltipTrigger>
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeSubTab} onValueChange={onSubTabChange} className="space-y-4">
        <div className="flex items-center justify-between mb-2">
          ...
          >
            <Plus size={14} strokeWidth={3} className={cn("transition-transform duration-300", isAdding && "rotate-45")} /> {isAdding ? "Cancel" : "New Snippet"}
          </Button>
        </div>
    ...
    <Card className="bg-[var(--accent-primary)]/[0.03] border border-[var(--accent-primary)]/20 ring-0 shadow-none p-5 animate-in fade-in slide-in-from-top-2 duration-300">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className="space-y-2">
          <label className="text-[10px] font-bold text-[var(--accent-primary)] tracking-wider">Terminal Command</label>
          <Input
            autoFocus
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-bold text-[var(--accent-primary)] tracking-wider">Snippet Label</label>
          <Input
      ...
      <div className="flex justify-end gap-2">
        <Button variant="ghost" size="sm" onClick={onCancel} className="text-[11px] h-8 text-[var(--text-secondary)]">Cancel</Button>
        <Button size="sm" onClick={onSave} className="bg-[var(--accent-primary)] text-[var(--accent-contrast)] text-[11px] font-bold h-8 hover:opacity-90">Save Snippet</Button>
      </div>
    </Card>
  );
}
    ...
    >
      ...
      <CardFooter className="px-4 py-2 border-t border-[var(--border-color)] bg-[var(--bg-color)]/20 flex items-center justify-between">
         <div className="flex items-center gap-2">
            ...
            {!onExecute && (
              <span className="text-[9px] text-[var(--text-secondary)] font-medium tracking-wider opacity-60">Terminal Snippet</span>
            )}
         </div>
         ...
         <div className="flex items-center gap-2.5">
            {onArchive && (
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="text-[10px]">Archive Snippet</TooltipContent>
              </Tooltip>
            )}
            {onRestore && (
              <Tooltip>
