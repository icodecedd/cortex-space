...
import { SpaceTemplate, Snippet } from "@/lib";
import {
  SavedLayout,
  LayoutConfig,
  INITIAL_LAYOUTS,
...
export function CortexLibraryDialog({
  ...
  const [selectedSnippetIds, setSelectedSnippetIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const [selectedLayoutIds, setSelectedLayoutIds] = useState<Set<string>>(
    new Set(),
  );
  const [archivedSelectedSnippetIds, setArchivedSelectedSnippetIds] = useState<
    Set<string>
  >(new Set());
  const [archivedSelectedTemplateIds, setArchivedSelectedTemplateIds] =
    useState<Set<string>>(new Set());
  const [archivedSelectedPresetIds, setArchivedSelectedPresetIds] = useState<
    Set<string>
  >(new Set());
  const [archivedSelectedLayoutIds, setArchivedSelectedLayoutIds] = useState<
    Set<string>
  >(new Set());
  ...
  const [layoutCustomMode, setLayoutCustomMode] = useState<"grid" | "count">(
    "grid",
  );
  ...
  useEffect(() => {
    async function loadSettingsMode() {
      const mode = await getSetting<"grid" | "count">(
  ...
  useEffect(() => {
    async function loadAssets() {
      const savedPresets = await getSetting<DirectoryPreset[]>(
        "cortex_presets",
        [],
      );
      ...
      const savedList = await getSetting<SavedLayout[]>(
  ...
  const handleArchivePreset = async (id: string) => {
    ...
    toast.info(`${name} archived`, {
      description: "The preset has been archived and can be restored later.",
    });
  };
  ...
  const handleUnarchivePreset = async (id: string) => {
    ...
    toast.success(`${name} restored`, {
      description: "The preset has been restored to your library.",
    });
  };
  ...
  const handleArchiveLayout = async (id: string) => {
    ...
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.info(`${name} archived`, {
      description: "The layout has been archived and can be restored later.",
    });
  };
  ...
  const handleUnarchiveLayout = async (id: string) => {
    ...
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.success(`${name} restored`, {
      description: "The layout has been restored to your library.",
    });
  };
  ...
  const handleDeletePreset = async (id: string) => {
    ...
    toast.success(`${name} permanently deleted`, {
      description: "The preset has been permanently removed.",
    });
  };
  ...
  const handleBulkUnarchivePresets = async (ids: string[]) => {
    const updated = presets.map((p) =>
      ids.includes(p.id) ? { ...p, isArchived: false } : p,
    );
    setPresets(updated);
    await setSetting("cortex_presets", updated);
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.success(`${ids.length} presets restored`, {
  ...
  const handleBulkDeletePresets = async (ids: string[]) => {
    ...
    await setSetting("cortex_presets", updated);
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.success(`${ids.length} presets permanently deleted`, {
  ...
  const handleBulkUnarchiveLayouts = async (ids: string[]) => {
    const updated = savedLayouts.map((l) =>
      ids.includes(l.id) ? { ...l, isArchived: false } : l,
    );
    setSavedLayouts(updated);
    await setSetting("cortex_saved_layouts", updated);
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.success(`${ids.length} layouts restored`, {
      description: `Successfully restored ${ids.length} items to your library.`,
    });
  };
  ...
  const handleBulkDeleteLayouts = async (ids: string[]) => {
    ...
    await setSetting("cortex_saved_layouts", updated);
    window.dispatchEvent(new Event("cortex:assets-updated"));
    toast.success(`${ids.length} layouts permanently deleted`, {
      description: `Successfully removed ${ids.length} items from your library.`,
    });
  };
  ...
  const handleAddPreset = async (label: string, path: string) => {
    if (presets.some((p) => p.path === path)) {
      toast.error("Preset cannot be added", {
        description:
          "An identical configuration already exists in your library.",
      });
      return;
    }
  ...
  const handleRestoreDefaults = async () => {
    ...
    if (toAdd.length === 0) {
      toast.success("Library updated successfully", {
        description:
          "Your library already contains all default configurations.",
      });
      return;
    }
  ...
  const toggleSelectAll = () => {
    if (activeTab === "commands") {
      ...
      const selectedIds =
        snippetSubTab === "active"
          ? selectedSnippetIds
          : archivedSelectedSnippetIds;
      const setSelectedIds =
        snippetSubTab === "active"
          ? setSelectedSnippetIds
          : setArchivedSelectedSnippetIds;
      ...
      const selectedIds =
        workspaceSubTab === "active"
          ? selectedTemplateIds
          : archivedSelectedTemplateIds;
      const setSelectedIds =
        workspaceSubTab === "active"
          ? setSelectedTemplateIds
          : setArchivedSelectedTemplateIds;
      ...
      const selectedIds =
        presetSubTab === "active"
          ? selectedPresetIds
          : archivedSelectedPresetIds;
      const setSelectedIds =
        presetSubTab === "active"
          ? setSelectedPresetIds
          : setArchivedSelectedPresetIds;
      ...
      const items = savedLayouts.filter(
        (l) =>
      ...
      const selectedIds =
        layoutSubTab === "active"
          ? selectedLayoutIds
          : archivedSelectedLayoutIds;
      const setSelectedIds =
        layoutSubTab === "active"
          ? setSelectedLayoutIds
          : setArchivedSelectedLayoutIds;
  ...
  const handleBulkArchive = () => {
    if (activeTab === "commands") {
      ...
      if (selectedPresetIds.size > 0) {
        const updated = presets.map((p) =>
          selectedPresetIds.has(p.id) ? { ...p, isArchived: true } : p,
        );
        setPresets(updated);
        setSetting("cortex_presets", updated);
        window.dispatchEvent(new Event("cortex:assets-updated"));
        toast.info(`${selectedPresetIds.size} presets archived`, {
          description:
            "The items have been archived and can be restored later.",
        });
      ...
      if (selectedLayoutIds.size > 0) {
        const updated = savedLayouts.map((l) =>
          selectedLayoutIds.has(l.id) ? { ...l, isArchived: true } : l,
        );
        setSavedLayouts(updated);
        setSetting("cortex_saved_layouts", updated);
        window.dispatchEvent(new Event("cortex:assets-updated"));
        toast.info(`${selectedLayoutIds.size} layouts archived`, {
          description:
            "The items have been archived and can be restored later.",
        });
        setSelectedLayoutIds(new Set());
        ...
        <div className="flex h-full">
          ...
          <div className="w-[220px] border-r border-[var(--border-color)] bg-[var(--bg-color)]/20 flex flex-col p-5 shrink-0">
            <div className="mb-10 px-1">
              ...
              <span className="text-[9px] text-[var(--text-secondary)] font-bold pl-1">
                Central Repository
              </span>
            </div>
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-[var(--text-secondary)]/75 font-medium">
                  {activeTab === "workspaces" &&
                    "Capture, launch, and restore workspace environments."}
                  {activeTab === "commands" &&
                    "Store, run, and inject frequently used command snippets."}
                  {activeTab === "presets" &&
            ...
            {((activeTab === "commands" &&
              (selectedSnippetIds.size > 0 ||
                archivedSelectedSnippetIds.size > 0)) ||
              (activeTab === "workspaces" &&
                (selectedTemplateIds.size > 0 ||
                  archivedSelectedTemplateIds.size > 0)) ||
              (activeTab === "presets" &&
                (selectedPresetIds.size > 0 ||
                  archivedSelectedPresetIds.size > 0)) ||
              (activeTab === "layouts" &&
                (selectedLayoutIds.size > 0 ||
                  archivedSelectedLayoutIds.size > 0))) &&
              (() => {
                ...
                if (activeTab === "commands") {
                  ...
                  const activeLayoutsForMode = savedLayouts.filter(
                    (l) => !l.isArchived && l.config.type === layoutCustomMode,
                  );
                  const archivedLayoutsForMode = savedLayouts.filter(
                    (l) => l.isArchived && l.config.type === layoutCustomMode,
                  );
                  if (layoutSubTab === "active" && selectedLayoutIds.size > 0) {
                    const selectedInMode = Array.from(selectedLayoutIds).filter(
                      (id) => activeLayoutsForMode.some((l) => l.id === id),
                    );
                    selectedCount = selectedInMode.length;
                    entityName = "Layouts";
                  } else if (
                    layoutSubTab === "archived" &&
                    archivedSelectedLayoutIds.size > 0
                  ) {
                    const selectedInMode = Array.from(
                      archivedSelectedLayoutIds,
                    ).filter((id) =>
                      archivedLayoutsForMode.some((l) => l.id === id),
                    );
                    selectedCount = selectedInMode.length;
                    entityName = "Layouts";
                    isArchivedContext = true;
                ...
                const handleGlobalBulkRestore = () => {
                  if (activeTab === "commands") {
                    if (
                      ...
                      onUnarchiveSnippets(
                        Array.from(archivedSelectedSnippetIds),
                      );
                    } else if (
                      onUnarchiveSnippet &&
                      archivedSelectedSnippetIds.size > 0
                    ) {
                      archivedSelectedSnippetIds.forEach((id) =>
                        onUnarchiveSnippet(id),
                      );
                    }
                  } else if (activeTab === "workspaces") {
                    if (
                      ...
                      onUnarchiveTemplates(
                        Array.from(archivedSelectedTemplateIds),
                      );
                    } else if (
                      onUnarchiveTemplate &&
                      archivedSelectedTemplateIds.size > 0
                    ) {
                      archivedSelectedTemplateIds.forEach((id) =>
                        onUnarchiveTemplate(id),
                      );
                    }
                  } else if (activeTab === "presets") {
                    handleBulkUnarchivePresets(
                      Array.from(archivedSelectedPresetIds),
                    );
                  } else if (activeTab === "layouts") {
                    handleBulkUnarchiveLayouts(
                      Array.from(archivedSelectedLayoutIds),
                    );
                  }
                ...
                const handleGlobalBulkDelete = () => {
                  if (activeTab === "commands") {
                    ...
                    handleBulkDeletePresets(
                      Array.from(archivedSelectedPresetIds),
                    );
                  } else if (activeTab === "layouts") {
                    handleBulkDeleteLayouts(
                      Array.from(archivedSelectedLayoutIds),
                    );
                  }
                  handleClearCurrentSelection();
        ...
        <span
          className={cn(
            ...
            isActive
              ...
              : "bg-[var(--text-primary)]/[0.05] text-[var(--text-secondary)]/60",
          )}
        >
          {count}
        </span>
