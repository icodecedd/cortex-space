...
export const ModeSelectorScreen = React.memo(
  ({
    onSelectMode,
    onBack,
            ...
            <div className="space-y-6">
              <motion.div variants={itemVariants} className="space-y-4">
                <h1 className="text-5xl md:text-7xl font-bold tracking-tighter text-[var(--text-primary)] leading-[0.9]">
                  ...
                  <br />
                  <span className="text-[var(--accent-primary)] brightness-110">
                    {" "}
                  ...
                  {showTemplatesHint && (
                    <div className="flex items-center gap-3 text-[var(--text-secondary)] text-xs font-bold hover:text-[var(--text-primary)] transition-colors cursor-default">
                      <KbdGroup className="gap-1">
                        {parseShortcutToKeys("Ctrl+Shift+T", isMac).map(
                          (key, idx) => (
                            <Kbd
                              key={idx}
                              className="bg-[var(--text-primary)]/5 border-[var(--border-color)]"
                            >
                              {key}
                            </Kbd>
                          ),
                      ...
                      <span className="tracking-tight">
                        {MODE_SELECTOR_CONTENT.HINTS.TEMPLATES}
                      </span>
                    </div>
                  )}
                  <div className="flex items-center gap-3 text-[var(--text-secondary)] text-xs font-bold hover:text-[var(--text-primary)] transition-colors cursor-default">
                    ...
                    <span className="tracking-tight">
                      {MODE_SELECTOR_CONTENT.HINTS.NEW_SPACE}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-[var(--text-secondary)] text-xs font-bold hover:text-[var(--text-primary)] transition-colors cursor-default">
                    <KbdGroup className="gap-1">
                    ...
                    <span className="tracking-tight">
                      {MODE_SELECTOR_CONTENT.HINTS.SETTINGS}
                    </span>
                  </div>
                </div>
              </motion.div>
            ...
            >
              <SpotlightCard
                className={`group cursor-pointer transition-all duration-500 ambient-glow-card !p-8 border-white/5 hover:border-[var(--accent-primary)]/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] ${
                  lastMode === "agents"
                    ? "active-glow !border-[var(--accent-primary)]/20"
                    : ""
                }`}
              >
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {showShortcutHints && (
                        <KbdGroup className="gap-1 opacity-30 group-hover:opacity-100 transition-opacity">
                          {parseShortcutToKeys("Ctrl+A", isMac).map(
                            (key, idx) => (
                              <Kbd
                                key={idx}
                ...
                <div className="flex items-center gap-8 relative">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {showShortcutHints && (
                        <KbdGroup className="gap-1 opacity-30 group-hover:opacity-100 transition-opacity">
                          {parseShortcutToKeys("Ctrl+N", isMac).map(
                            (key, idx) => (
                              <Kbd
                                key={idx}
                                className="bg-[var(--text-primary)]/5 border-[var(--border-color)] text-[10px] px-1 h-4 flex items-center justify-center font-mono"
