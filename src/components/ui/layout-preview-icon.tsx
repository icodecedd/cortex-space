...
export function LayoutPreviewIcon({
  ...
  const renderNode = (
    ...
    return (
      ...
      >
        <div style={{ flex: ratio }}>{renderNode(children[0])}</div>
        <div style={{ flex: 1 - ratio }}>{renderNode(children[1])}</div>
      </div>
    );
  };
  ...
  return (
    <div
      className={cn(
        "w-12 h-10 flex border border-[var(--border-color)] rounded-[4px] overflow-hidden p-[1px] bg-[var(--bg-color)]/40",
        className,
      )}
    >
