...
import { LayoutNode, SplitNode, PaneNode, Agent } from "@/lib";
...
export function configToLayoutNode(
  config: LayoutConfig,
  panes: PaneConfig[],
): LayoutNode {
  if (config.type === "grid") {
...
export function extractNode(
  root: LayoutNode,
  targetId: string,
): { newRoot: LayoutNode | null; extracted: PaneNode | null } {
  if (root.type === "pane") {
    if (root.id === targetId) {
  ...
  const { newRoot: left, extracted: leftExtracted } = extractNode(
    root.children[0],
    targetId,
  );
  const { newRoot: right, extracted: rightExtracted } = extractNode(
    root.children[1],
    targetId,
  );
...
export function findNeighborPane(
  ...
  const parentMap = new Map<
    LayoutNode,
    { parent: LayoutNode; index: number }
  >();
  ...
  if (mappedTool) {
    ...
    if (pattern) {
      ...
      const sub = parts.find(
        (p) => !p.startsWith("-") && pattern.sub.includes(p.toLowerCase()),
      );
      if (sub) {
